...
```bash
for f in */*/*/*/*.pileup.txt.gz ; do mkdir -p $(echo $f | cut -d'/' -f1-3)/ivarconsensus && zcat $f | ivar consensus -p $(echo $f | sed 's/\/pileup\//\/ivarconsensus\//g' | sed 's/\.txt\.gz$/.ivar/g') -m 10 -n N -t 0.5 ; done
```
